...
import {
    Input, InputGroup, InputGroupText, InputGroupAddon, FormGroup, Col, Label,
} from 'reactstrap';
        ...
        formLayout = this.state.formData.map((que, id) => {
            return (
                <div>
                    <FormGroup row>
                        <Col xs="12">
                            <h4>{id} : {que.question}</h4>
                        </Col>
                    </FormGroup>
                    <FormGroup row>
                        <Col xs="12">
                            {this.displayAnswerFields(que)}
                        </Col>
                    </FormGroup>
                </div>
            );
        ...
        if(que.inputType === 'Text') {
            ...
            return (
                que.options.map((opt) => {
                    return (
                        <FormGroup check inline>
                            <Input className="form-check-input" type="radio" name="inline-radios" value={opt.value} />
                            <Label className="form-check-label" style={{fontSize : 20}} check htmlFor="inline-radio1">{opt.value}</Label>
                        </FormGroup>
                    )
                })
            )
