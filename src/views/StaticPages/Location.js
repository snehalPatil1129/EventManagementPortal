...
class EventLocation extends Component {
    constructor(props) {
        ...
        this.state = {
            eventLocation: {
                ...
                longitude: 0, latitudeDelta: 0.0922, longitudeDelta: 0.0421
            },
            eventRequired: false, addressRequired: false, cordinateError: ''
        }
    }
    componentWillMount() {
    ...
    onSubmit() {
        let eventLocation = { ...this.state.eventLocation };
        if (eventLocation.event && eventLocation.latitude && eventLocation.longitude && eventLocation.address) {
            let isEmpty = !Object.keys(this.props.eventLocation).length;
            let eventLocation = _.pick(this.state.eventLocation, ['event', 'latitude', 'longitude', 'address', 'latitudeDelta', 'longitudeDelta']);
            if (isEmpty) { //post
            ...
            !eventLocation.address ? this.setState({ eventEmailRequired: true }) : null;
            !eventLocation.latitude || !eventLocation.longitude ? this.setState({ cordinateError: 'Please Submit Again' }) : null
        }
    }
    onReset() {
        this.setState(prevState => ({
    ...
    render() {
        ...
        return (
            <CardLayout name="Event Location">
                ...
                <FormGroup row>
                    <Col xs="12" md="3">
                        <Button type="button" size="md" color="success" onClick={() => this.getCordinates()} >Submit</Button>
                    </Col>
                    <Col md="3">
                    ...
                    <Col md="6">
                        ...
                        {
                            <div style={{ color: "red" }} className="help-block">{this.state.cordinateError}</div>
                        }
                    </Col>
                </FormGroup >
            </CardLayout>
...
const mapDispatchToProps = dispatch => {
    return {
        ...
        createEventLocation: (eventLocation) => dispatch(actions.createEventLocation(eventLocation)),
        editEventLocation: (id, eventLocation) => dispatch(actions.editEventLocation(id, eventLocation)),
    };
}
