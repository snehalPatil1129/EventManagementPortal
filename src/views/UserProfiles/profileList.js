...
class profileList extends Component {
    ...
    handleEventSelectChange(value) {
        let eventValue = value, profiles = [];
        this.setState({
            eventValue: eventValue
        });
        this.props.profiles.forEach(profile => {
            if (profile.eventId == eventValue)
    ...
    onDeleteProfile(cell, row) {
        ...
        return <Link to={this} onClick={() => componentRef.deleteProfile(row.id)} >
    ...
    onEditProfile(cell, row) {
        let componentRef = this;
        return <Link to={`${componentRef.props.match.url}/profileForm/${row.id}`}>
    ...
    render() {
        ...
        const sortingOptions = {
            ...
            sizePerPageList: [{
                ...
                text: 'All', value: this.state.profiles.length
            }],
            sizePerPage: 250,
        };
        return (
            <div>
                ...
                <CardLayout name="Profile List">
                    ...
                    <FormGroup row>
                        <BootstrapTable ref='table' data={this.state.profiles} pagination={true} search={true} options={sortingOptions} exportCSV={true}>
                            <TableHeaderColumn dataField='id' headerAlign='left' isKey hidden>Id</TableHeaderColumn>
                            <TableHeaderColumn dataField='profileName' headerAlign='left' width='100' csvHeader='Profile Name'>Profile Name</TableHeaderColumn>
                            <TableHeaderColumn dataField='delete' dataFormat={this.onDeleteProfile.bind(this)} headerAlign='left' width='30' export={false}></TableHeaderColumn>
                            <TableHeaderColumn dataField='edit' dataFormat={this.onEditProfile.bind(this)} headerAlign='left' width='30' export={false}></TableHeaderColumn>
                        </BootstrapTable>
                    </FormGroup>
                </CardLayout>
            </div>
